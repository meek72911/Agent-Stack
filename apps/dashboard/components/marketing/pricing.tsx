          ...
          {plans.map((plan, i) => (
            <motion.div 
              ...
              className={cn(
                ...
                plan.highlighted 
                  ? "border-[rgba(249,115,22,0.5)] bg-gradient-to-br from-[rgba(249,115,22,0.08)] to-[#0D0F17] shadow-premium hover:shadow-[0_0_80px_rgba(249,115,22,0.25)] hover:scale-[1.02]" 
                  : "border-[#1C1F2E] bg-[#0D0F17] hover:border-[rgba(249,115,22,0.3)] hover:shadow-[0_0_40px_rgba(249,115,22,0.1)] hover:scale-[1.01]"
              )}
            >
              {plan.highlighted && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <Badge className="bg-gradient-premium text-[#07080C] font-bold px-5 py-1 text-xs shadow-glow-sm" style={{ borderRadius: '999px' }}>
                    MOST POPULAR
                  </Badge>
                </div>
              )}
                ...
                {plan.features.map((feature) => (
                  <li key={feature.name} className="flex items-center gap-3 text-sm">
                    {feature.included 
                      ...
                      : <X className="h-4 w-4 shrink-0 text-[#3F4558]" />
                    }
                    <span className={feature.included ? "text-[#94A3B8]" : "text-[#3F4558]"}>
                      {feature.name}
                    </span>
                  </li>
                ))}
